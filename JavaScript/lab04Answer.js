...
function genId () {
  ...
  return Math.floor(Math.random() * (9_999_999 - 9_000_000) + 9_000_000);
}
...
const genStudentObject  = (Name) => ({name : Name , id :  genId()});
...
const firstStudent = studentObjects.reduce((min, current) => min.id > current.id ? current : min);
...
// const countTcss = setStudents.reduce((Count, X) => X.major == 'TCSS' ? Count++ : Count);


const countTcss = setStudents.reduce(((Counter, X) => X.major == 'TCSS' ? Counter + 1 : Counter),0);
